...
const WorkdaysComponent = () => {
  ...
  useEffect(() => {
    axios
      .get(`${import.meta.env.BACKEND_URL}/workdays`,{headers:{"token":localStorage.getItem("token")}})
      .then((response) => {
  ...
  const handleWorkdaysSubmit = (e) => {
    ...
    axios
      .put(`${import.meta.env.BACKEND_URL}/workdays`, updatedDays,{headers:{"token":localStorage.getItem("token")}})
      .then(() => {
